  ...
  render() {
    ...
    if (this.context.gameState.isTie) {
      ...
      if (this.context.gameType === GAME_TYPES.TWO_PLAYERS) {
        if (this.context.gameState.position === "") {
          textInfo = `Au tour du joueur(${ICON_CHARS[currentIconType]})`;
        } else {
          textInfo = `Victoire du joueur(${ICON_CHARS[1 - currentIconType]})!`;
        }
      } else {
        if (this.context.gameState.position === "") {
          if (this.context.playerTurn === PLAYER_TURNS.HUMAN) textInfo = `Votre tour`;
          else textInfo = `Tour de l'IA`;
        } else {
          if (this.context.playerTurn === PLAYER_TURNS.HUMAN) textInfo = `Victoire IA!`;
          else textInfo = `Vous avez gagné!`;
        }
      }
    }
